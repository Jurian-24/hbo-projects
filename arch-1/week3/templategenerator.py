...
def jobChecker(job):
    if(len(job) < 10):
        print('Error: Invalid amount of characters. Your job needs to have atleast 10 characters')
        job = input('Enter your job \n >>> ')
        jobChecker(job)
    if(any(char.isdigit() for char in job)):
        print('Error: You can`t enter a number in your job. Try again.')
        job = input('Enter your job  \n >>> ')
        jobChecker(job)
...
def salaryCheck(salary):
    ...
    if(len(salary) < 9):
        ...
        print('Error: You cant have a value below 20.000,00 or higher than 80.000,00')
        salary = input('Enter your salary in this format: 20.000,00: ')
        salaryCheck(salary)
...
def createJobOfferMail(firstName, lastName, job):
    ...
    finalJobOfferEmail = f"""
        Here is the final letter to send: Dear {firstName.capitalize()} {lastName.capitalize()},
...
def dateChecker(date):
    ...
    if(int(day) > maxDays):
        ...
        print('ERROR: You can only enter a date in the years 2021 or 2022')
        date = input('Starting date (format: YYYY-MM-DD): ')
        dateChecker(date)
    elif(0 >= int(month) > 12):
        print('ERROR: You can`t enter a date below 0 or bigger than 12.')
        date = input('Starting date (format: YYYY-MM-DD): ')
        dateChecker(date)
...
wantToWriteAMail = input('Do you want to write an email? Yes/no: ')

if(wantToWriteAMail.lower() == 'yes'):
    startApp()
else:
    exit()
